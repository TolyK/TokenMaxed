// Generated by packages/openclaw-plugin/build.mjs — DO NOT EDIT. TokenMaxed OpenClaw plugin (banner + routing gate + finalize review).
...
// ../mcp/src/openclaw-plugin.ts
var OPENCLAW_DELEGATE_TOOL = "tokenmaxed__router_delegate";
function openclawPluginEnv(processEnv) {
...
function finalizeDecisionFor(action, maxRounds) {
...
var FINALIZE_HOOK_TIMEOUT_MS = REVIEW_CHILD_KILL_MS + 1e4;
function makeFinalizeHandler(deps) {
  const reviewing = /* @__PURE__ */ new Set();
  const counter = deps.counter ?? fileLoopCounter;
  const keylessKey = `keyless-${randomUUID2()}`;
  return async (payload) => {
    try {
      const env = deps.env();
      if (!reviewLoopEnabled(env)) return void 0;
      const key = sessionKeyFrom(payload);
      const sessionID = key ?? keylessKey;
      if (reviewing.has(sessionID)) return void 0;
...
var tokenMaxedOpenclawPlugin = {
  id: "tokenmaxed",
  name: "TokenMaxed",
  description: "Quota-continuity router: offload right-sized subtasks to the cheapest capable, policy-allowed lane. Banner + routing gate + turn-end review.",
  register(api) {
    const bannered = /* @__PURE__ */ new Set();
    const remember = (id) => {
      bannered.add(id);
      if (bannered.size > MAX_TRACKED_SESSIONS) {
        const oldest = bannered.values().next().value;
        if (oldest !== void 0) bannered.delete(oldest);
      }
    };
    const pluginEnv = () => openclawPluginEnv(process.env);
    const reviewScript = fileURLToPath3(new URL("./tokenmaxed-review.mjs", import.meta.url));
    ...
    api.on("before_prompt_build", async (payload) => {
      try {
        ...
        if (bannered.has(key)) return void 0;
        remember(key);
        const env = pluginEnv();
        if (env.TOKENMAXED_DISABLE === "1" || env.TOKENMAXED_DISABLE === "true") return void 0;
        if (!bannerWithinBudget(env)) return void 0;
        const banner = clampBanner(formatSummaryBanner(await makeSummaryFromEnv(env)()));
        return banner.trim() ? { prependContext: banner } : void 0;
      } catch {
        return void 0;
      }
    });
    ...
    const handleFinalize = makeFinalizeHandler({
      env: pluginEnv,
      runReview: (sessionID, env) => spawnReviewChild(reviewScript, sessionID, env, { killAfterMs: FINALIZE_HOOK_TIMEOUT_MS - 1e4 }),
      surface
    });
    api.on("before_agent_finalize", handleFinalize, { timeoutMs: FINALIZE_HOOK_TIMEOUT_MS });
  }
};
var openclaw_plugin_default = tokenMaxedOpenclawPlugin;
export {
