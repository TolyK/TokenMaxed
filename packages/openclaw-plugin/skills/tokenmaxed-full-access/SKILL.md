...
# /tokenmaxed_full_access

Grant a SPECIFIC reader-trust lane full access to this repo — the scoped, per-lane
alternative to `/tokenmaxed_yolo`. When granted, that reader lane becomes selectable
...
Take the user's argument (everything after `/tokenmaxed_full_access`, trimmed, case-insensitive):

- If it begins with `off`, `none`, `clear`, `revoke`, or `remove`:
  - With a trailing model name (e.g. `off minimax`): call
    `tokenmaxed__router_set_full_access` with
...
models. Use `/tokenmaxed_status` or `/tokenmaxed_why <category>` to see the effect.
