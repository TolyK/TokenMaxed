...
Take the user's argument (everything after `/tokenmaxed_yolo`, trimmed, case-insensitive):

- If the argument is empty, or equals `on`, `yes`, or `enable`, call
  `tokenmaxed__router_set_yolo` with `{ "enabled": true }`.
- If the argument equals `off`, `no`, `disable`, `clear`, or `none`, call
  `tokenmaxed__router_set_yolo` with `{ "enabled": false }`.
...
`/tokenmaxed_status` to see whether YOLO is currently on, or `/tokenmaxed_why <category>`
