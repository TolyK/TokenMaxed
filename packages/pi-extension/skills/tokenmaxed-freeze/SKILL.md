...
Take the user's argument (everything after `/skill:tokenmaxed-freeze`, trimmed, case-insensitive):

- If the argument is empty, or equals `on`, `yes`, or `enable`, call
  `tokenmaxed_router_set_freeze` with `{ "enabled": true }`.
- If the argument equals `off`, `no`, or `disable`, call
  `tokenmaxed_router_set_freeze` with `{ "enabled": false }`.
