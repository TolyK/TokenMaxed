...
# /skill:tokenmaxed-yolo

YOLO mode is the TokenMaxed equivalent of Claude Code's `--dangerously-skip-permissions`:
...
Take the user's argument (everything after `/skill:tokenmaxed-yolo`, trimmed, case-insensitive):

- If the argument is empty, or equals `on`, `yes`, or `enable`, call
  `tokenmaxed_router_set_yolo` with `{ "enabled": true }`.
- If the argument equals `off`, `no`, `disable`, `clear`, or `none`, call
  `tokenmaxed_router_set_yolo` with `{ "enabled": false }`.
...
`/skill:tokenmaxed-status` to see whether YOLO is currently on, or `/skill:tokenmaxed-why <category>`
