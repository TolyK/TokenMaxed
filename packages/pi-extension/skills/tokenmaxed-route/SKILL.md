...
1. Call the MCP tool `tokenmaxed_router_delegate` with the
...
Respect the toggle: if the user ran `/skill:tokenmaxed-off`, do not offload. The user
can check state with `/skill:tokenmaxed-status` and re-enable with `/skill:tokenmaxed-on`.
