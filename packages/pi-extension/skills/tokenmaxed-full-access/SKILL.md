...
# /skill:tokenmaxed-full-access

Grant a SPECIFIC reader-trust lane full access to this repo — the scoped, per-lane
alternative to `/skill:tokenmaxed-yolo`. When granted, that reader lane becomes selectable
...
Take the user's argument (everything after `/skill:tokenmaxed-full-access`, trimmed, case-insensitive):
...
models. Use `/skill:tokenmaxed-status` or `/skill:tokenmaxed-why <category>` to see the effect.
