...
# /skill:tokenmaxed-reserve

Take the user's argument (everything after `/skill:tokenmaxed-reserve`, trimmed).
...
The capacity reservation is persisted per project and survives restarts; no relaunch is needed to turn it on or off. Run `/skill:tokenmaxed-status` to see whether capacity reservations are active, or `/skill:tokenmaxed-why <category>` to preview reservation pressure.
