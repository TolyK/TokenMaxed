...
- If the argument is empty, or equals `off`, `none`, or `clear` (case-insensitive), call
  `mcp__plugin_tokenmaxed_tokenmaxed__router_set_policy` with `{}`. This resets the policy to default "balanced" behavior.
- Otherwise, call `mcp__plugin_tokenmaxed_tokenmaxed__router_set_policy` with
