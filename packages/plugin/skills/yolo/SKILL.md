...
YOLO mode is the TokenMaxed equivalent of Claude's `--dangerously-skip-permissions`:
...
- If the argument is empty, or equals `on`, `yes`, or `enable`, call
  `mcp__plugin_tokenmaxed_tokenmaxed__router_set_yolo` with `{ "enabled": true }`.
- If the argument equals `off`, `no`, `disable`, `clear`, or `none`, call
  `mcp__plugin_tokenmaxed_tokenmaxed__router_set_yolo` with `{ "enabled": false }`.
