// Generated by packages/opencode-plugin/build.mjs — DO NOT EDIT. TokenMaxed OpenCode plugin (banner + routing gate + review loop).
...
// ../mcp/src/opencode-plugin.ts
var OPENCODE_DELEGATE_TOOL = "tokenmaxed_router_delegate";
var REWORK_PROMPT_PREFIX = "[TokenMaxed review \u2014 rework requested] ";
function opencodePluginEnv(processEnv, directory) {
...
var TokenMaxed = async (input) => {
  ...
  const pluginEnv = () => opencodePluginEnv(process.env, input.directory);
  const reviewScript = fileURLToPath3(new URL("./tokenmaxed-review.mjs", import.meta.url));
  const toast = async (message, variant = "warning") => {
    try {
  ...
  const handleIdle = makeIdleReviewHandler({
    ...
    ...promptFn ? {
      promptBack: (sessionID, text) => promptFn({ path: { id: sessionID }, body: { parts: [{ type: "text", text }] } })
    } : {},
    toast
  });
  return {
    ...
    "tool.execute.before": async (toolInput) => {
      ...
      if (reason) throw new Error(denyReasonForHost(reason, "-"));
    },
    ...
    event: async ({ event }) => {
      try {
        ...
        const sessionID = typeof props.sessionID === "string" ? props.sessionID : typeof props.info?.id === "string" ? props.info.id : void 0;
