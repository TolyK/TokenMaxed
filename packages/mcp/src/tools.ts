...
export interface CorePort {
  ...
  /** Pure confidence-bounded quota estimate fusion. */
  quotaEstimate?: (lane: Lane, events: readonly LedgerEvent[], opts: { calibrationFraction?: number; routedShare?: number } | undefined, now: number) => QuotaEstimate;
  /**
  ...
  assessDeprecation: (modelId: string, table: PriceTable, now: number) => DeprecationReport;
  resolveDeprecatedModel: <L extends { model: string }>(lane: L, table: PriceTable, now: number) => { lane: L; migratedFrom?: string; warning?: string };
  resolveLaneModel?: <L extends { model: string }>(lane: L, table: PriceTable) => L;
}
...
export interface ToolDeps {
  ...
  /** Whether capability learning is frozen for the project. */
  getFrozen?: () => boolean;
  /** Set or clear the project's frozen capability learning state. */
  ...
  /** Read the project's manual routed shares. */
  getRoutedShares?: () => Record<string, number>;
  /** Set (lane/model, fraction) or clear manual routed shares. */
  setRoutedShare?: (lane: string | undefined, fraction: number | undefined) => void;
...
function resolveCalibrationFractionLocal(core: CorePort, lane: Lane, calibrations: Record<string, number>): number | undefined {
...
function resolveRoutedShareFractionLocal(core: CorePort, lane: Lane, routedShares: Record<string, number>): number | undefined {
  const laneIdLower = lane.id.toLowerCase();
  const laneModelLower = lane.model.toLowerCase();
...
function formatQuotaEstimateText(
  modelOrId: string,
  est: QuotaEstimate,
  routedShare: number | undefined,
): string {
  ...
  if (est.dominantSource === 'routed') {
    if (est.reportedFraction === undefined && est.inferredFraction === undefined) {
  ...
  if (est.dominantSource === 'inferred') {
    const sharePct = routedShare !== undefined ? Math.round(routedShare * 100) : 100;
    return `${modelOrId}: ${routedPct}% routed ÷ ~${sharePct}% routed-share ⇒ est. ~${pointEstimatePct}% used (inferred, ${est.confidence})`;
  }

  // est.dominantSource === 'reported'
  const calPct = est.reportedFraction !== undefined ? Math.round(est.reportedFraction * 100) : 0;
...
function policyExplanation(
  ...
  const previewTool: ToolDef = {
    ...
    handler: (deps, args) =>
      guardedAsync(async () => {
        ...
        let priceTableCached: PriceTable | undefined;
        let priceTableLoaded = false;
        const getPriceTable = (): PriceTable | undefined => {
          if (!priceTableLoaded) {
            priceTableCached = deps.loadPriceTable ? deps.loadPriceTable() : undefined;
        ...
        for (const lane of lanes) {
          if (lane.trust_mode === 'reader') {
            ...
            const allLanes = deps.allLanes ? deps.allLanes() : [];
            const origModel = allLanes.find((x) => x.id === lane.id)?.model;
            const matchesPromptPin = !!full_access && !!pinnedModel && (
              core.modelMatchesPin(lane.model, pinnedModel) || (origModel && core.modelMatchesPin(origModel, pinnedModel))
            );
        ...
        const yoloNote = yolo
          ...
          : undefined;
        const freezeNote = deps.getFrozen?.()
          ? '  learning: frozen for this project (outcome learning is paused; routing falls back to declared/prior capability without recent outcomes).'
          : undefined;
        ...
        if (instruction) {
          ...
          if (lane && core.forecastCost) {
            try {
              const forecast = core.forecastCost(combinedText, lane, getPriceTable());
              ...
              // price table load or forecast failure shouldn't fail the preview
            }
          }
        }
        ...
        if (core.resolveLaneModel) {
          ...
          if (allLanes.length > 0) {
            try {
              ...
              // ignore loadPriceTable throwing/missing
            }
          }
        }
  ...
  const statusTool: ToolDef = {
    ...
    handler: (deps) =>
      guardedAsync(async () => {
        ...
        let userFeedbackEvents: OutcomeEvent[] = [];
        try {
          userFeedbackEvents = (deps.readLedger?.() ?? []).filter(
            (e): e is OutcomeEvent => e.event_type === 'outcome' && e.voter === 'user'
          );
        } catch {
          // ignore ledger read errors for status tool
        }
        ...
        for (const [key, val] of Object.entries(routedShares)) {
          if (typeof val === 'number' && Number.isFinite(val) && val > 0 && val <= 1) {
            const pct = Math.round(val * 100);
            shareLines.push(`  ${key}: estimated routed share ${pct}%`);
          }
        }
        ...
        if (enabled && core.quotaEstimate && (Object.keys(calibrations).length > 0 || Object.keys(routedShares).length > 0)) {
          ...
          const quotaLanes = allLanes.filter(
            (l) =>
              (typeof l.requests_per_window === 'number' && l.requests_per_window > 0) ||
              (typeof l.requests_per_week === 'number' && l.requests_per_week > 0) ||
              (typeof l.tokens_per_week === 'number' && l.tokens_per_week > 0)
          );
                ...
                for (const l of allLanes) {
                  ...
                  if (report.status === 'deprecated') {
                    ...
                    deprecationPayload.push({
                      ...
                      successorPriced: report.successor !== undefined && Object.hasOwn(priceTable.models, report.successor),
    ...
    inputSchema: {
      ...
      properties: {
        lane: {
          type: 'string',
          description:
            'The lane ID or model name to set routed share for (e.g. claude-native or opus). If empty, clears all routed shares for this project.',
        },
        share: {
          type: 'string',
          description:
            'The estimated routed share fraction (e.g. "30%" or "0.3"). Use "off", "none", or "clear" to remove the routed share for this lane.',
        },
      },
    },
    handler: (deps, args) =>
      guarded(() => {
        ...
        const shareStr = typeof rawShare === 'string' ? rawShare.trim().toLowerCase() : undefined;

        if (!lane) {
          deps.setRoutedShare?.(undefined, undefined);
          return ok(
        ...
        if (!shareStr || ['off', 'none', 'clear'].includes(shareStr)) {
          deps.setRoutedShare?.(lane, undefined);
          return ok(
        ...
        const resolvedNames = matchedLanes.map((l) => `${l.id} (${l.model})`).join(', ');
        return ok(
          `Estimated routed share of ${pct}% set for: ${resolvedNames} in this project.`,
  ...
  const feedbackTool: ToolDef = {
    ...
    description:
      'Record direct user feedback on routing / offload quality to adapt future decisions. The feedback verdict can apply to the LAST offload in the ledger or target an explicit lane + category (+ optional difficulty). Opinions are marked as user feedback and feed learned capabilities exactly like manager reviews. Content-free.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['verdict'],
    ...
    handler: (deps, args) =>
      guarded(() => {
        ...
        const verdict = optEnum(args, 'verdict', ['good', 'wrong-model', 'bad-output', 'too-slow'] as const);
        ...
        if ('category' in args) {
          ...
          if (typeof catVal !== 'string' || !core.taskCategories.includes(catVal as any)) {
        ...
        if ('difficulty' in args) {
          ...
          if (typeof diffVal !== 'string' || !DIFFICULTIES.includes(diffVal as any)) {
        ...
        if ((args.lane !== undefined || args.category !== undefined) && (!explicitLane || !explicitCategory)) {
          throw new ToolInputError('Explicit targeting requires both "lane" and "category".');
        }

        let subjectLaneId: string;
        let subjectModel: string;
        let subjectModelResolved: string;
        let category: TaskCategory;
        let difficulty: DifficultyBucket | undefined;
        let taskId: string;
        let attempt = 0;
        ...
        if (explicitLane && explicitCategory) {
          ...
          try {
            ...
            throw new Error(`Failed to read ledger: ${(err as any).message}`);
          }
          if (!ledger || ledger.length === 0) {
            throw new ToolInputError('No recent offloads found in the ledger to rate (ledger is empty).');
          }
          let lastOffload: LedgerEvent | undefined;
        ...
        // Map verdict to ReviewVerdict: good => pass, wrong-model/bad-output => fail, too-slow => needs-rework
        let mappedVerdict: ReviewVerdict;
        if (verdict === 'good') {
          mappedVerdict = 'pass';
        ...
        const outcomeInput: OutcomeEventInput = {
          subject_id: taskId,
        ...
        const targetMsg = explicitLane
          ? `explicitly for lane "${subjectLaneId}" (category ${category}${difficulty ? `, difficulty ${difficulty}` : ''})`
          : `for the last offload on lane "${subjectLaneId}" (task ${taskId}, category ${category}${difficulty ? `, difficulty ${difficulty}` : ''})`;
  ...
  const setFreezeTool: ToolDef = {
    name: 'router_set_freeze',
    description:
      'Freeze or unfreeze TokenMaxed capability outcome learning for this project. When frozen, accumulated outcome learning is paused, and routing falls back to declared/prior capability without recent outcomes. User feedback and reviews are still recorded in the ledger but will not affect routing. Persisted per project. Powers /tokenmaxed:freeze.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['enabled'],
    ...
    handler: (deps, args) =>
      guarded(() => {
        const enabled = optBool(args, 'enabled');
        if (enabled === undefined) throw new ToolInputError('"enabled" is required.');
        if (!deps.setFrozen) {
          throw new Error('Project settings write is not supported by this host.');
        }
        deps.setFrozen(enabled);
        const msg = enabled
          ? 'Capability outcome learning is FROZEN for this project (routing falls back to declared/prior capability without recent outcomes).'
          : 'Capability outcome learning is UNFROZEN for this project (outcome learning is active/resumed).';
