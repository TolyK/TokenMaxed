#!/usr/bin/env node
// Generated by packages/hermes-plugin/build.mjs — DO NOT EDIT. TokenMaxed pre_llm_call banner (Hermes).
...
// ../mcp/src/hook-hermes-prellmcall.ts
var import_node_fs10 = require("node:fs");

// ../mcp/src/hermes-hooks.ts
var import_node_crypto = require("node:crypto");
var import_node_fs = require("node:fs");
var import_node_os = require("node:os");
var import_node_path = require("node:path");
function hermesSessionKey(payload) {
  const raw = typeof payload.session_id === "string" && payload.session_id ? payload.session_id : "default";
  return raw.replace(/[^A-Za-z0-9_.-]/g, "_") + "-" + (0, import_node_crypto.createHash)("sha1").update(raw).digest("hex").slice(0, 8);
}
function claimBannerMarker(sessionKey, dir = (0, import_node_path.join)((0, import_node_os.tmpdir)(), "tokenmaxed-banner")) {
  try {
    const marker = (0, import_node_path.join)(dir, sessionKey);
    if ((0, import_node_fs.existsSync)(marker)) return false;
    (0, import_node_fs.mkdirSync)(dir, { recursive: true });
    (0, import_node_fs.writeFileSync)(marker, "1", { flag: "wx" });
    return true;
  } catch {
    return false;
  }
...
function loadLaneConfig(path) {
  const filePath = typeof path === "string" ? path : (0, import_node_url.fileURLToPath)(path);
  let text;
  try {
    text = (0, import_node_fs2.readFileSync)(filePath, "utf8");
  } catch (err) {
    const detail = err instanceof Error ? err.message : String(err);
...
function defaultLedgerPath() {
  return (0, import_node_path2.join)((0, import_node_os2.homedir)(), ".tokenmaxed", "ledger.jsonl");
}
function readCliUsageByModel(projectDir) {
  ...
  try {
    ...
    const tdir = (0, import_node_path2.join)((0, import_node_os2.homedir)(), ".claude", "projects", encoded);
    if (!(0, import_node_fs2.existsSync)(tdir)) return out;
    let budget = 64 * 1024 * 1024;
    for (const f of (0, import_node_fs2.readdirSync)(tdir)) {
      if (!f.endsWith(".jsonl")) continue;
      let text;
      try {
        text = (0, import_node_fs2.readFileSync)((0, import_node_path2.join)(tdir, f), "utf8");
      } catch {
        continue;
      }
...
var JsonlLedger = class {
  ...
  #load() {
    ...
    const text = (0, import_node_fs2.readFileSync)(this.path, "utf8");
  ...
  #write(event, events) {
    ...
    (0, import_node_fs2.appendFileSync)(this.path, prefix + serializeEvent(event) + "\n", "utf8");
...
var HOME_TM = (0, import_node_path3.join)((0, import_node_os3.homedir)(), ".tokenmaxed");
function homeFile(name) {
  return (0, import_node_path3.join)(HOME_TM, name);
}
function makeLoadPolicy(env) {
  ...
  return () => {
    if ((0, import_node_fs3.existsSync)(policyPath)) return loadPolicyConfig(policyPath);
...
function spawnPath(execPath = process.execPath, base = process.env.PATH) {
  const binDir = (0, import_node_path3.dirname)(execPath);
  const parts = (base ?? "").split(import_node_path3.delimiter).filter(Boolean);
  if (parts.includes(binDir)) return parts.join(import_node_path3.delimiter);
  return [binDir, ...parts].join(import_node_path3.delimiter);
}
...
function isExecutableFile(candidate) {
  try {
    if (!(0, import_node_fs4.statSync)(candidate).isFile()) return false;
...
function commandOnPath(command, path) {
  ...
  return dirs.some((dir) => isExecutableFile((0, import_node_path4.join)(dir, command)));
}
function regularFileExists(candidate) {
  try {
    return (0, import_node_fs4.statSync)(candidate).isFile();
  } catch {
    return false;
  }
}
...
function loadSettings(env) {
  ...
  if (!(0, import_node_fs5.existsSync)(path)) return { values: {}, present: false, invalid: [] };
  let parsed;
  try {
...
function readFreshnessCache(path) {
  try {
    return (0, import_node_fs6.existsSync)(path) ? coerceCache(JSON.parse((0, import_node_fs6.readFileSync)(path, "utf8"))) : emptyCache();
  } catch {
    return emptyCache();
  }
}
...
function laneSetFingerprint(lanes) {
  ...
  return (0, import_node_crypto3.createHash)("sha256").update(JSON.stringify(canonical)).digest("hex");
}
...
function readLaneReviewState(path) {
  try {
    return (0, import_node_fs7.existsSync)(path) ? coerceLaneReviewState(JSON.parse((0, import_node_fs7.readFileSync)(path, "utf8"))) : emptyLaneReviewState();
  } catch {
...
function readOnlyToggleStore(statePath) {
  return {
    read: () => {
      try {
        return (0, import_node_fs8.existsSync)(statePath) ? JSON.parse((0, import_node_fs8.readFileSync)(statePath, "utf8")) : {};
      } catch {
        return {};
      }
    },
...
function makeSummaryFromEnv(env) {
  ...
  const statePath = env.TOKENMAXED_STATE ?? (env.CLAUDE_PLUGIN_DATA ? (0, import_node_path5.join)(env.CLAUDE_PLUGIN_DATA, "state.json") : homeFile("state.json"));
  ...
  const pricesPath = env.TOKENMAXED_PRICES ?? (0, import_node_url2.fileURLToPath)(new URL("../prices.seed.json", __importMetaUrl));
  const cachePath = env.TOKENMAXED_MODEL_CACHE ?? (0, import_node_path5.join)((0, import_node_path5.dirname)(statePath), "model-freshness.json");
  const reviewProjectKey = env.TOKENMAXED_PROJECT ?? env.CLAUDE_PROJECT_DIR ?? "default";
  const laneStatePath = env.TOKENMAXED_LANE_STATE ?? (0, import_node_path5.join)((0, import_node_path5.dirname)(statePath), "lane-review.json");
  const meteredKeyWarning = !!(env.ANTHROPIC_API_KEY && env.ANTHROPIC_API_KEY.trim());
  return async () => {
    const lanes = (0, import_node_fs8.existsSync)(lanesPath) ? [...loadLaneConfig(lanesPath).lanes] : [];
    const available = await probeAvailable(lanes);
    const now = Date.now();
    let priceTable;
    if ((0, import_node_fs8.existsSync)(pricesPath)) {
      try {
...
function bannerWithinBudget(env) {
  try {
    const path = env.TOKENMAXED_LEDGER ?? (0, import_node_path6.join)((0, import_node_os4.homedir)(), ".tokenmaxed", "ledger.jsonl");
    if (!(0, import_node_fs9.existsSync)(path)) return true;
    return (0, import_node_fs9.statSync)(path).size <= BANNER_LEDGER_MAX_BYTES;
  } catch {
    return true;
  }
}
...
async function main() {
  let payload = {};
  try {
    payload = JSON.parse((0, import_node_fs10.readFileSync)(0, "utf8") || "{}");
  } catch {
  }
