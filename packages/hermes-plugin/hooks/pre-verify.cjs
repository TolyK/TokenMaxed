#!/usr/bin/env node
// Generated by packages/hermes-plugin/build.mjs — DO NOT EDIT. TokenMaxed pre_verify review (Hermes).
...
// ../mcp/src/hook-hermes-preverify.ts
...
function stopGateDecision(verdict, priorBlocks, maxAttempts) {
...
var HERMES_VERIFY_BUDGET_MS = 26e4;
var HERMES_MAX_VERIFY_NUDGES_DEFAULT = 3;
function hermesMaxRounds(env) {
  const raw = env.TOKENMAXED_HERMES_VERIFY_NUDGES;
  const nudges = raw !== void 0 && /^[0-9]+$/.test(raw) && Number.parseInt(raw, 10) >= 1 ? Number.parseInt(raw, 10) : HERMES_MAX_VERIFY_NUDGES_DEFAULT;
  return Math.min(parseMaxRounds(env), nudges);
}
function hermesVerifyAttempt(payload) {
...
function loadLaneConfig(path) {
  ...
  let text;
  try {
    text = (0, import_node_fs.readFileSync)(filePath, "utf8");
  } catch (err) {
    const detail = err instanceof Error ? err.message : String(err);
...
function defaultLedgerPath() {
  return (0, import_node_path.join)((0, import_node_os.homedir)(), ".tokenmaxed", "ledger.jsonl");
}
var JsonlLedger = class {
  ...
  #load() {
    ...
    const text = (0, import_node_fs.readFileSync)(this.path, "utf8");
  ...
  #write(event, events) {
    ...
    (0, import_node_fs.appendFileSync)(this.path, prefix + serializeEvent(event) + "\n", "utf8");
...
function makeCliExecutor(spawnImpl) {
  const spawn = spawnImpl ?? ((cmd, args, opts) => (0, import_node_child_process.spawnSync)(cmd, [...args], opts));
...
var HOME_TM = (0, import_node_path2.join)((0, import_node_os2.homedir)(), ".tokenmaxed");
function homeFile(name) {
  return (0, import_node_path2.join)(HOME_TM, name);
}
function makeLoadPolicy(env) {
  ...
  return () => {
    if ((0, import_node_fs2.existsSync)(policyPath)) return loadPolicyConfig(policyPath);
...
function spawnPath(execPath = process.execPath, base = process.env.PATH) {
  const binDir = (0, import_node_path2.dirname)(execPath);
  const parts = (base ?? "").split(import_node_path2.delimiter).filter(Boolean);
  if (parts.includes(binDir)) return parts.join(import_node_path2.delimiter);
  return [binDir, ...parts].join(import_node_path2.delimiter);
}
function makeCliSpawn(timeoutMs = DEFAULT_CLI_TIMEOUT_MS) {
  return (command, args, options) => {
    ...
    const res = (0, import_node_child_process2.spawnSync)(command, [...args], spawnOptions);
    if (typeof res.pid === "number") {
      try {
        if (process.platform === "win32") {
          (0, import_node_child_process2.spawnSync)("taskkill", ["/pid", String(res.pid), "/t", "/f"]);
        } else {
...
function isExecutableFile(candidate) {
  try {
    if (!(0, import_node_fs3.statSync)(candidate).isFile()) return false;
...
function commandOnPath(command, path) {
  ...
  return dirs.some((dir) => isExecutableFile((0, import_node_path3.join)(dir, command)));
}
function regularFileExists(candidate) {
  try {
    return (0, import_node_fs3.statSync)(candidate).isFile();
  } catch {
    return false;
  }
}
...
function readUntrackedDiff(cwd) {
  const list = (0, import_node_child_process3.spawnSync)("git", ["ls-files", "--others", "--exclude-standard", "-z"], {
    cwd,
    encoding: "utf8",
  ...
  for (const f of all) {
    ...
    const d = (0, import_node_child_process3.spawnSync)("git", ["diff", "--no-index", "--", "/dev/null", f], {
      cwd,
      encoding: "utf8",
...
function makeHostReviewDeps(env, opts = {}) {
  ...
  return {
    readDiff: () => {
      const res = (0, import_node_child_process3.spawnSync)("git", ["diff", "HEAD"], { cwd, encoding: "utf8", maxBuffer: GIT_MAX_BUFFER, timeout: GIT_TIMEOUT_MS });
...
[NOTE: review coverage is INCOMPLETE \u2014 omitted: ${gaps.join("; ")}]`;
    ...
    loadLanes: () => {
      if (!(0, import_node_fs4.existsSync)(lanesPath)) return null;
    ...
    newId: () => (0, import_node_crypto2.randomUUID)()
  };
}
...
async function main() {
  let payload = {};
  try {
    payload = JSON.parse((0, import_node_fs6.readFileSync)(0, "utf8") || "{}");
  } catch {
  }
  ...
  const result = await runReviewWithBudget(makeReviewRunner(makeHostReviewDeps(env, { totalBudgetMs: HERMES_VERIFY_BUDGET_MS })), import_node_crypto3.randomUUID, {
  ...
  const action = stopHookAction({
    ...
    priorBlocks: hermesVerifyAttempt(payload),
    // the HOST tracks nudges this turn
    maxRounds: hermesMaxRounds(env)
    // min(TokenMaxed cap, Hermes nudge default) — see hermes-hooks.ts
  });
